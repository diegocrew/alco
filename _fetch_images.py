...
def find(name):
    d = get({
        ...
        "gsrsearch": f"{name} cocktail", "gsrnamespace": "6", "gsrlimit": "8",
...
def main():
    names = json.load(open(os.path.join(ROOT, "_names.json"), encoding="utf-8"))
    os.makedirs(OUT_DIR, exist_ok=True)

    cred_path = os.path.join(ROOT, "_credits.json")
    credits = {}
    ...
    for i, name in enumerate(names, 1):
        ...
        if s in credits and os.path.exists(dest_raw):
            continue
